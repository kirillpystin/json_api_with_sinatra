...
describe 'Test app' do
  ...
  it 'Add Genius(id:1)', type: :request do
    ...
    puts 'Step-1: Crazy Genius-1 saved'
  end
  ...
  it 'Add Genius(id: 2)', type: :request do
		...
		expect(last_response.status).to eq(302)
    puts 'Step-2: Crazy Genius-2 saved'
  end

	it 'Add Invent for Genius(id: 2)', type: :request do
		header = {'Content-Type' => 'application/json'}
		post '/crazy/2/add', { 'name_arm':'Teleportator', 'power':40 }.to_json, header
		expect(last_response.status).to eq(200)
		puts 'Step-4: Invent-1 from Genius(id:2) saved'
  end
	...
	it 'Add Invent for Genius(id: 2)', type: :request do
		...
		expect(last_response.status).to eq(200)
		puts 'Step-4: Invent-2 from Genius(id:2) saved'
	end
  ...
  it 'update Genius(id: 2)', type: :request do
		...
		expect(last_response.status).to eq(200)
		puts 'Step-7: Genius(id:1) updated'
  end
  # Update invention
  ...
  it 'delete Genius(id:1) with all his Inventions' do
		delete '/crazy/1'
		expect(last_response.status).to eq(302)
		puts 'Step-9: Genius(id:1) and Invention was deleted'
  end

	it 'Delete Invent(id:5) from Genius(id: 2)' do
		delete '/crazy/2/5'
		expect(last_response.status).to eq(302)
		puts 'Step-10: Invent(id:5) from Genius(id:2) was deleted'
	end
end
